...
onsetDffAvg = []; offsetDffAvg = []; onsetDff = []; offsetDff = []; combinedDff = []; combinedDffAvg =[]; respRawF = []; baselineRawF = []; combinedRawF = [];
...
for iType = 1:nEventTypes
    ...
    clear('respData'); clear('baselineData')
...
%% EXTRACT ROI DATA FOR FILTERED EVENTS

baselineROIAvgOnset = []; respROIAvgOnset = []; baselineROIAvgOffset = []; respROIAvgOffset = []; combinedBaselineROIAvg = []; combinedRespROIAvg = []; ROIDffOnset = []; ROIDffOffset = [];
for iType = 1:nEventTypes
    ...
    for iCond = 1:nConds
        ...
        if sum(onsetFilterVecs{iType}(:,iCond)) > 0
            ...
            currBaselineROIAvg = []; currRespROIAvg = []; ROIDff = [];
            for iROI = 1:nROIs  
                disp(['ROI #', num2str(iROI), ' of ', num2str(nROIs), ' onset...'])
                ...
                ROIDff(:,:,iROI) = (currRespROIAvg - baselineRep) ./ baselineRep;                       % --> [volume, event]
            end%if
            baselineROIAvgOnset{iType}{iCond} = currBaselineROIAvg;                                     % --> {eventType}{cond}[volume, event, ROI]
            respROIAvgOnset{iType}{iCond} = currRespROIAvg;                                             % --> {eventType}{cond}[volume, event, ROI]
            ROIDffOnset{iType}{iCond} = ROIDff;                                                         % --> {eventType}{cond}[volume, event, ROI]
            
        end%if
        
        % Get ROI data for event offsets
        if sum(offsetFilterVecs{iType}(:,iCond)) > 0
            ...
            currBaselineROIAvg = []; currRespROIAvg = []; ROIDff = [];
            for iROI = 1:nROIs  
                disp(['ROI #', num2str(iROI), ' of ', num2str(nROIs), ' offset...'])
                currMask = myData.ROIdata(iROI).mask;
                currPlane = myData.ROIdata(iROI).plane;
                nEvents = size(baselineData, 5);
            ...
            baselineROIAvgOffset{iType}{iCond} = currBaselineROIAvg;                                    % --> {eventType}{cond}[volume, event, ROI]
            respROIAvgOffset{iType}{iCond} = currRespROIAvg;                                            % --> {eventType}{cond}[volume, event, ROI]
            ROIDffOffset{iType}{iCond} = ROIDff;                                                        % --> {eventType}{cond}[volume, event, ROI]

        end%if
        
    end%iCond
    combinedBaselineROIAvg{iType} = cat(1,  baselineROIAvgOnset{iType},  baselineROIAvgOffset{iType});  % --> {eventType}{cond}[volume, event, ROI]
    combinedRespROIAvg{iType} = cat(1,  respROIAvgOnset{iType},  respROIAvgOffset{iType});              % --> {eventType}{cond}[volume, event, ROI]
    combinedROIDffAvg{iType} = cat(1, ROIDffOnset{iType}, ROIDffOffset{iType});                         % --> {eventType}{cond}[volume, event, ROI]
    
    clear('respData', 'baselineData', 'baselineROIAvgOnset', 'baselineROIAvgOffset', 'respROIAvgOnset', 'respROIAvgOffset', 'ROIDffOnset', 'ROIDffOffset');
...
% combinedDff --> {eventType}[y, x, plane, volume, condition]
disp(annotationTypeSummary(activeEventTypes, 2))
odorEventDff = combinedDff{1}; % --> [y, x, plane, volume, condition]

for iROI = 1:nROIs
    currMask = myData.ROIdata(iROI).mask;
    currPlane = myData.ROIdata(iROI).plane;
    currPlaneData = squeeze(odorEventDff(:,:,currPlane,:,:)); % --> [y, x, volume, condition]
    currPlaneData(~currMask(:,:,ones(1, nVolumes), ones(1, nTrials))) = nan;                    % --> [ y, x, volume, trial]
...
fileNameSuffix = '_AllTrials';%'_OdorAvsOdorBvsAirPulse'; %
for iFold = 1
...
make_heatmap_vid(trialAvg, myData, range, fileName, titleStrings, savePath, [], [], [], smoothingSigma);
    ...
    savePath = ['B:\Dropbox (HMS)\2P Data\Imaging Data\', expDate, '\sid_0\Analysis'];
