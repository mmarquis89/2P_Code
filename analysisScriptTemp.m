...
for iFoldIn = 1 
    ...
    % Plot figures
    [~, ~] = plot_heatmaps(onsetMeanDff, myData, range, plotTitle, [], [], smoothingSigma, makeVid, saveDir, fileName);
    ...
    % Create video
    savePath = ['D:\Dropbox (HMS)\2P Data\Imaging Data\', expDate, '\sid_', myData.sid, '\Analysis'];
...
for iFoldIn = 1
    ...
    for iTrial = 1:nTrials
        if isempty(strfind(excludeTrials, iTrial))
            ...
            ballStopFrames = [ballStop', ballRelease'];     % --> [ballStop, ballRelease]
            currBallStopVols = zeros(size(ballStopFrames));
             for iTime = 1:numel(ballStopFrames)
                [~, currBallStopVols(iTime)] = min(abs(volFrames - ballStopFrames(iTime))); % --> [ballStop, ballRelease]
             end
            currBallStopVols(:,3) = iTrial;                     % --> [ballStop, ballRelease, trial]
            ballStopVols = [ballStopVols; currBallStopVols];    % --> [ballStop, ballRelease, trial]
        end
    end
    
    % Find minimum duration of each annotation period
    ballStopDur = min(ballStopVols(:,2) - ballStopVols(:,1));
    
    % Drop any ball-stopping events that are too close to the trial onset or offset
    ballStopVols(ballStopVols(:,1) < ballStopDur, :) = [];
    ballStopVols(ballStopVols(:,2) > (nVolumes - ballStopDur), :) = [];
...
%     end
    ...
    [f, plotAxes] = plot_heatmaps(dffCurrConds, myData, range, plotTitles(currConds), [], [], smoothingSigma, makeVid, saveDir, fileName); %trialCondNames(currConds)
